...
- the measurable facts — counts, timings, the run they came from
- an acceptance criterion, written as **"Fertig, wenn …" / "Done when …"**
- one label from the small set below

Labels: `bug`, `ui`, `wörterbuch`, `harness`, `ux-idea`, plus `prio-hoch` when it
goes before the rest. The backend repo uses the same set with `scraper` in place
of `ui`.

Measure before you name a cause. "Probably X" belongs in an issue; it does not
...
- no install IDs, no account identifiers, no certificate names
...
Decisions that are the maintainer's to make — a colour, a product direction, a
release call — are not issues. They live in the private roadmap and only become
issues once they are decided.
